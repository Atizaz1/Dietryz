...
{
  UpdateFood({@required this.title, @required this.foodId});
...
class _UpdateFoodState extends State<UpdateFood> 
{
  ...
  String foodName, foodGroup, dateConsumed, mealType, notes, reporterName, foodId;
  ...
  void formSubmit() async
  {
    ...
    if (form.validate()) 
    {
      ...
      if(fetchedFoodObject.foodName != foodName)
      {
        int duplicateCheck = await dbHelper.findIfFoodExists(foodName);
        ...
        else if(duplicateCheck <=0)
        {
          ...
          int check  = await dbHelper.updateFood(inputFoodObject, foodId);
  ...
  void setFoodId(dynamic foodId)
  {
    this.foodId = foodId;
  }

  void fetchFoodData(String foodId) async
  {
    fetchedFoodObject      = await dbHelper.readFood(foodId);

    foodNameController.text     = fetchedFoodObject.foodName;

    foodGroupController.text    = fetchedFoodObject.foodGroup;

    mealTypeController.text     = fetchedFoodObject.mealType;

    dateConsumedController.text = fetchedFoodObject.dateConsumed;

    notesController.text        = fetchedFoodObject.notes;

    reportController.text       = fetchedFoodObject.reporterName;

    print(fetchedFoodObject);
  }
  ...
  void initState() 
  {
    ...
    setFoodId(widget.foodId);
    fetchFoodData(this.foodId);
  }
